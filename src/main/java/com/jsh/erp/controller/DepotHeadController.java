...
import com.jsh.erp.datasource.entities.DepotHeadAndPriceDetailsBody;
...
public class DepotHeadController {
    ...
    public String batchSetStatus(@RequestBody JSONObject jsonObject,
        ...
        int res = depotHeadService.batchSetStatus(status, ids, comment, request);
        if (res > 0) {
    ...
    public BaseResponseInfo findInOutDetail(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            List<DepotHeadVo4InDetail> list = depotHeadService.findInOutDetail(beginTime, endTime, type, creatorArray, supplierId, forceFlag,
                    StringUtil.toNull(materialParam), depotList, client, StringUtil.toNull(number), remark, (currentPage - 1) * pageSize, pageSize);
            int total = depotHeadService.findInOutDetailCount(beginTime, endTime, type, creatorArray, supplierId, forceFlag,
                    StringUtil.toNull(materialParam), depotList, client, StringUtil.toNull(number), remark);
            map.put("total", total);
            //存放数据json数组
            if (null != list) {
    ...
    public Object addDepotHeadAndDetail(@RequestBody DepotHeadAndPriceDetailsBody body, HttpServletRequest request) throws Exception {
        JSONObject result = ExceptionConstants.standardSuccess();
        String beanJson = body.getInfo();
        String rows = body.getRows();
        PriceDetailsVo priceDetailsVo = body.getPriceDetailsVo();
        List<SupplierServeProject> supplierServeProjects = body.getSupplierServeProjects();
        String originType = body.getOriginType();
        depotHeadService.addDepotHeadAndDetail(beanJson, rows, priceDetailsVo, supplierServeProjects, originType,  request);
        return result;
    }
    ...
    public Object updateDepotHeadAndDetail(@RequestBody DepotHeadAndPriceDetailsBody body, HttpServletRequest request) throws Exception {
        JSONObject result = ExceptionConstants.standardSuccess();
        String beanJson = body.getInfo();
        String rows = body.getRows();
        PriceDetailsVo priceDetailsVo = body.getPriceDetailsVo();
        List<SupplierServeProject> supplierServeProjects = body.getSupplierServeProjects();
        depotHeadService.updateDepotHeadAndDetail(beanJson, rows, priceDetailsVo, supplierServeProjects, request);
        return result;
    }
    ...
    @ApiOperation(value = "根据当前用户获取商品状态数组")
    public BaseResponseInfo getListByStatus(@RequestParam String subType,
                                            @RequestParam Long supplierId,
                                            @RequestParam String[] statusArray,
                                            @RequestParam("currentPage") Integer currentPage,
                                            @RequestParam("pageSize") Integer pageSize) {
        BaseResponseInfo res = new BaseResponseInfo();
        Map<String, Object> map = new HashMap<>();
        List<DepotHeadVo6List> list = depotHeadService.getListByStatus(subType, supplierId, statusArray, (currentPage - 1) * pageSize, pageSize);
        int total = depotHeadService.getCountByStatus(subType, supplierId, statusArray);
        if (list != null) {
            res.code = 200;
            map.put("rows", list);
            map.put("total", total);
     ...
     * @return
     */
    @GetMapping(value = "/getBuyAndSaleStatisticsBySupplier")
    @ApiOperation(value = "统计结算费用")
    public BaseResponseInfo getBuyAndSaleStatisticsBySupplier(@RequestParam(value = "type") String type,
        ...
        endTime = Tools.parseDayToTime(endTime, BusinessConstants.DAY_LAST_TIME);
        try {
            BigDecimal totalAmount = depotHeadService.getBuyAndSaleStatisticsBySupplier(type, subType, supplierId, beginTime, endTime);
            res.code = 200;
