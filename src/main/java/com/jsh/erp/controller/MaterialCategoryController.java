...
public class MaterialCategoryController {
        ...
        try {
            ...
            if (null != dataList) {
                for (MaterialCategory mc : dataList) {
                    ...
                    outer.put("deleteFlag", mc.getDeleteFlag());
                    outer.put("tenantId", mc.getTenantId());
    ...
    public JSONArray getMaterialCategoryTree(@RequestParam("id") Long id, @RequestParam("type") String type,
                                             @RequestParam(value = "importEditFlag", required = false) String importEditFlag) throws Exception {
        JSONArray arr = new JSONArray();
        List<TreeNode> materialCategoryTree = materialCategoryService.getMaterialCategoryTree(id, type, importEditFlag);
    ...
    @GetMapping(value = "/checkCategoryNameIsExist")
    @ApiOperation(value = "检查类别名称是否重复")
    public int checkCategoryNameIsExist(@RequestParam("id") Long id, @RequestParam("name") String name,
                                        @RequestParam("type") String type, HttpServletRequest request) throws Exception {
        return materialCategoryService.checkCategoryNameIsExist(id, name, type, request);
    }

    @PostMapping(value = "/importExcel")
    @ApiOperation(value = "excel表格导入分类")
    public BaseResponseInfo importExcel(MultipartFile file,
                                        HttpServletRequest request, HttpServletResponse response) throws Exception {
        BaseResponseInfo res = new BaseResponseInfo();
        try {
            res = materialCategoryService.importExcel(file, request);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return res;
    ...
    @PostMapping(value = "/batchUpdateMaterialCategory")
    @ApiOperation(value = "批量修改商品类别上级目录")
    public Object batchUpdateMaterialCategory(@RequestParam("ids") List<Long> ids, @RequestParam(value = "parentId", required = false) Long parentId,
                                              @RequestParam(value = "remark", required = false) String remark) throws Exception {
        BaseResponseInfo res = new BaseResponseInfo();
        try {
            res = materialCategoryService.batchUpdateMaterialCategory(ids, parentId, remark);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return res;
    ...
    @GetMapping(value = "/getNumberByLevel")
    @ApiOperation(value = "通过等级查询最新的编号")
    public String getNumberByLevel(@RequestParam("categoryLevel") Integer categoryLevel, HttpServletRequest request) throws Exception {
        return materialCategoryService.getNumberByLevel(categoryLevel, request);
    }
}
