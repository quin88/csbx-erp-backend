...
public class MaterialController {
    ...
    public JSONObject findBySelect(@RequestParam(value = "supplierId", required = false) Long supplierId,
                                   ...
                                   @RequestParam(value = "zeroStockFilter", required = false) Integer zeroStockFilter,//过滤0库存商品,0：不过滤，1：过滤
        ...
        try {
            ...
            List<MaterialVo4Unit> dataList = materialService.findBySelectWithBarCode(supplierId, categoryId, q, enableSerialNumber,
                    enableBatchNumber, zeroStockFilter, (currentPage - 1) * pageSize, pageSize);
            int total = materialService.findBySelectWithBarCodeCount(supplierId, categoryId, q, enableSerialNumber, enableBatchNumber, zeroStockFilter);
            ...
            if (null != dataList) {
                for (MaterialVo4Unit material : dataList) {
                    ...
                    item.put("weight", material.getWeight());
    ...
    public BaseResponseInfo getListWithStock(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            List<MaterialVo4Unit> dataList = materialService.getListWithStock(depotList, idList, StringUtil.toNull(materialParam), zeroStock,
                    StringUtil.safeSqlParse(column), StringUtil.safeSqlParse(order), supplierId, beginTime, endTime,
                    (currentPage - 1) * pageSize, pageSize);
            int total = materialService.getListWithStockCount(depotList, idList, StringUtil.toNull(materialParam), zeroStock, supplierId);
            MaterialVo4Unit materialVo4Unit = materialService.getTotalStockAndPrice(depotList, idList, StringUtil.toNull(materialParam), supplierId);
            map.put("total", total);
    ...
    @PostMapping(value = "/batchSetApprovalStatus")
    @ApiOperation(value = "批量设置审核状态")
    public String batchSetApprovalStatus(@RequestBody JSONObject jsonObject,
                                         HttpServletRequest request) throws Exception {
        String status = jsonObject.getString("status");
        String ids = jsonObject.getString("ids");
        String comment = jsonObject.getString("comment");
        Map<String, Object> objectMap = new HashMap<>();
        int res = materialService.batchSetApprovalStatus(status, comment, ids, request);
        if (res > 0) {
            return returnJson(objectMap, ErpInfo.OK.name, ErpInfo.OK.code);
        } else {
            return returnJson(objectMap, ErpInfo.ERROR.name, ErpInfo.ERROR.code);
