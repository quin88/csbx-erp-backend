 ...
 * 工具类
 *
 * @author CSBX  qq:7-5-2-7-1-8-9-2-0
 */
public class Tools {
    /**
     * 获得32位唯一序列号
    ...
    public static String getCenternTime(Date date) {
       ...
       //todo 时间问题
        sdf.setTimeZone(TimeZone.getTimeZone("Asia/Shanghai")); // 直接指定上海时区
...
//	}
     ...
     * @return
     */
    public static Date convertToUTC(Date date, TimeZone fromTimeZone) {
        Calendar fromCalendar = Calendar.getInstance(fromTimeZone);
        fromCalendar.setTime(date);

        Calendar utcCalendar = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
        utcCalendar.setTimeInMillis(fromCalendar.getTimeInMillis() - fromTimeZone.getRawOffset());
