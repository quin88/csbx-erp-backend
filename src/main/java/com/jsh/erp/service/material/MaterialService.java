...
public class MaterialService {
    ...
    @Value("${MATERIAL.CHECK.LINK}")
    private String material_check_link;
    ...
    public List<MaterialVo4Unit> select(String materialParam, String color, String materialOther, String weight, String expiryNum,
                                        ...
                                        String remark, String categoryId, String mpList, String approvalStatus, Long supplierId, String originType,
                                        int offset, int rows)
            throws Exception {
        String[] mpArr = new String[]{};
        String[] statusArray = StringUtil.isNotEmpty(approvalStatus) ? approvalStatus.split(",") : null;
        ...
        try {
            ...
            list = materialMapperEx.selectByConditionMaterial(materialParam, color, materialOther, weight, expiryNum,
                    enableSerialNumber, enableBatchNumber, enabled, remark, idList, mpList, statusArray, supplierId, originType, offset, rows);
    ...
    public Long countMaterial(String materialParam, String color, String materialOther, String weight, String expiryNum,
                              String enableSerialNumber, String enableBatchNumber, String enabled,
                              String remark, String categoryId, String mpList, String approvalStatus, Long supplierId, String originType) throws Exception {
        Long result = null;
        String[] statusArray = StringUtil.isNotEmpty(approvalStatus) ? approvalStatus.split(",") : null;
        try {
            List<Long> idList = new ArrayList<>();
            ...
            result = materialMapperEx.countsByMaterial(materialParam, color, materialOther, weight, expiryNum,
                    enableSerialNumber, enableBatchNumber, enabled, remark, idList, mpList, statusArray, supplierId, originType);
        } catch (Exception e) {
            JshException.readFail(logger, e);
        }
        return result;
    ...
    public int insertMaterial(JSONObject obj, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            String supplierName = supplierMapper.selectByPrimaryKey(m.getSupplierId()).getSupplier();
            String originType = m.getOriginType();
            Date now = new Date();
            Long tenantId = userService.getCurrentUser().getTenantId();
            String status = m.getApprovalStatus();
            ...
            if (tenantId == 146 && BusinessConstants.ORIGIN_TYPE.equals(originType) &&
                ...
                String content = "审核页面：商品审核" + "\n" +
                        "客户：" + supplierName + "\n" +
                        "来源：" + originType + "\n" +
                        "时间：" + Tools.parseDateToStr(now) + "\n" +
                        "链接：" + material_check_link;
    ...
    public int updateMaterial(JSONObject obj, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            String supplierName = supplierMapper.selectByPrimaryKey(material.getSupplierId()).getSupplier();
            String originType = material.getOriginType();
            Date now = new Date();
            Long tenantId = userService.getCurrentUser().getTenantId();
            String status = material.getApprovalStatus();
            //如果租户为城市佰兴，来源为小程序，则发送钉钉通知操作员审核(小程序群)
            if (tenantId == 146 && BusinessConstants.ORIGIN_TYPE.equals(originType)&&
                    status.equals(BusinessConstants.BILLS_STATUS_UN_AUDIT)) {
                String content = "审核页面：商品审核" + "\n" +
    ...
    public int batchSetStatus(Boolean status, String ids) throws Exception {
        ...
        material.setVerifier(userService.getCurrentUser().getId());
        MaterialExample example = new MaterialExample();
    ...
    public List<MaterialVo4Unit> findBySelectWithBarCode(Long supplierId, Long categoryId, String q, String enableSerialNumber,
                                                         String enableBatchNumber, Integer zeroStockFilter, Integer offset, Integer rows) throws Exception {
        List<MaterialVo4Unit> list = null;
        try {
            List<Long> idList = new ArrayList<>();
            if (categoryId != null) {
            ...
            list = materialMapperEx.findBySelectWithBarCode(supplierId, idList, q, enableSerialNumber, enableBatchNumber, zeroStockFilter, offset, rows);
        } catch (Exception e) {
            JshException.readFail(logger, e);
        }
        return list;
    ...
    public int findBySelectWithBarCodeCount(Long supplierId, Long categoryId, String q, String enableSerialNumber,
                                            String enableBatchNumber, Integer zeroStockFilter) throws Exception {
        int result = 0;
        try {
            List<Long> idList = new ArrayList<>();
            if (categoryId != null) {
            ...
            result = materialMapperEx.findBySelectWithBarCodeCount(supplierId, idList, q, enableSerialNumber, enableBatchNumber, zeroStockFilter);
        } catch (Exception e) {
    ...
    public void exportExcel(String categoryId, String materialParam, String color, String weight,
        ...
        String nameStr = "供应商*,名称*,规格,型号,颜色,类别,基础重量(kg),保质期(天),基本单位*,副单位,基本条码*,副条码,比例,采购价,零售价,销售价,最低售价,状态*,序列号,批号,备注";
    ...
    public BaseResponseInfo importExcel(MultipartFile file, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            for (int i = 2; i < rightRows; i++) {
                String supplier = ExcelUtils.getContent(src, i, 0); //供应商
                ...
                if (supplier == null || supplier.isEmpty()) {
                    throw new BusinessRunTimeException(ExceptionConstants.MATERIAL_SUPPLIER_OVER_CODE,
                            String.format(ExceptionConstants.MATERIAL_SUPPLIER_OVER_MSG, i + 1));
                }
                Long supplierId = supplierMapperEx.selectSupplierIdBySupplier(supplier);
                if (supplierId == null) {
                    throw new BusinessRunTimeException(ExceptionConstants.MATERIAL_SUPPLIER_OVER_CODE,
                            String.format(ExceptionConstants.MATERIAL_SUPPLIER_OVER_MSG, i + 1));
                }
                ...
                String remark = ExcelUtils.getContent(src, i, 20); //备注
                m.setRemark(remark);
                //状态格式错误
                ...
                m.setApprovalStatus(BusinessConstants.BILLS_STATUS_UN_AUDIT);
                mList.add(m);
            }
    ...
    public List<MaterialVo4Unit> getListWithStock(List<Long> depotList, List<Long> idList, String materialParam, Integer zeroStock,
                                                  String column, String order, Long supplierId, String beginTime, String endTime, Integer offset, Integer rows) throws Exception {
        ...
        List<MaterialVo4Unit> dataList = materialMapperEx.getListWithStock(depotList, idList, materialParam, zeroStock, column, order, supplierId,
    ...
    public int getListWithStockCount(List<Long> depotList, List<Long> idList, String materialParam, Integer zeroStock, Long supplierId) {
        return materialMapperEx.getListWithStockCount(depotList, idList, materialParam, zeroStock, supplierId);
    }

    public MaterialVo4Unit getTotalStockAndPrice(List<Long> depotList, List<Long> idList, String materialParam, Long supplierId) {
        return materialMapperEx.getTotalStockAndPrice(depotList, idList, materialParam, supplierId);
    }
     ...
     */
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public int batchSetApprovalStatus(String status, String comment, String ids, HttpServletRequest request) throws Exception {
        logService.insertLog("商品",
                new StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_EDIT).append(ids).toString(),
                ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest());
        List<Long> materialIds = StringUtil.strToLongList(ids);
        ...
        for (Long id : materialIds) {
            ...
            String appStatus = material.getApprovalStatus();
            if (statusRules.containsKey(status) && statusRules.get(status).contains(appStatus)) {
                mIds.add(id);
            } else {
        ...
        Material material = new Material();
        material.setApprovalStatus(status);
