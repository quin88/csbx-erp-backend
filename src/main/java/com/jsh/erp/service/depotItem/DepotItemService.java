...
public class DepotItemService {
     ...
     * @throws Exception
     */
    public DepotItem getPreItemByHeaderIdAndMaterial(Long preHeaderId, Long meId, Long linkId) throws Exception {
        DepotItem depotItem = new DepotItem();
        try {
            DepotHead depotHead = depotHeadService.getDepotHead(preHeaderId);
            DepotItemExample example = new DepotItemExample();
    ...
    public List<DepotItemVo4WithInfoEx> getDetailList(String headerId) throws Exception {
        ...
        try {
            String[] headIdArr = headerId.split(",");
            list = depotItemMapperEx.getDetailList(headIdArr);
        } catch (Exception e) {
            JshException.readFail(logger, e);
        }
        return list;
            ...
            for (int i = 0; i < rowArr.size(); i++) {
                ...
                if (StringUtil.isNotEmpty(depotHead.getLinkNumber())
                    ...
                    if ("add".equals(actionType)) {
                        ...
                        if ("出库".equals(depotHead.getType()) && depotItem.getOperNumber().add(finishNumber).compareTo(preNumber) > 0) {
                        ...
                        String unit = rowObj.get("unit").toString();
                        //查询之前的订单数据
                        DepotItem depotItemOrder = depotItemMapper.selectByPrimaryKey(depotItem.getLinkId());
                        Long preHeaderId = depotItemOrder.getHeaderId();
                        //前一个单据的数量，查询关联订单数据
                        BigDecimal preNumber = getPreItemByHeaderIdAndMaterial(preHeaderId, depotItem.getMaterialExtendId(), depotItem.getLinkId()).getOperNumber();
                        //除去此单据之外的已入库|已出库
                ...
                if (StringUtil.isExist(rowObj.get("depotId"))) {
                    ...
                    if (!BusinessConstants.SUB_TYPE_SELF_ORDER.equals(depotHead.getSubType())
                            //自营订单、寄存订单、销售自营、销售寄存订单放行
                            && !BusinessConstants.SUB_TYPE_CONSIGNMENT_ORDER.equals(depotHead.getSubType())
                            && !BusinessConstants.SUB_TYPE_SALE_CONSIGNMENT_ORDER.equals(depotHead.getSubType())
                            && !BusinessConstants.SUB_TYPE_SALE_SELF_ORDER.equals(depotHead.getSubType())
                ...
                if (StringUtil.isExist(rowObj.get("totalWeight"))) {
                    depotItem.setTotalWeight(rowObj.getBigDecimal("totalWeight"));
                }
                if (StringUtil.isExist(rowObj.get("productionDate"))) {
                    depotItem.setProductionDate(rowObj.getDate("productionDate"));
                }
                if (StringUtil.isExist(rowObj.get("producingArea"))) {
                    depotItem.setProducingArea(rowObj.getString("producingArea"));
                }
                if (StringUtil.isExist(rowObj.get("numberOfPanels"))) {
                    depotItem.setNumberOfPanels(rowObj.getBigDecimal("numberOfPanels"));
                }
                if (StringUtil.isExist(rowObj.get("numberOfPallets"))) {
                    depotItem.setNumberOfPallets(rowObj.getBigDecimal("numberOfPallets"));
                }
                ...
                if (StringUtil.isExist(rowObj.get("goodsAllocationId"))) {
                    depotItem.setGoodsAllocationId(rowObj.getLong("goodsAllocationId"));
                }
                if (StringUtil.isExist(rowObj.get("anotherGoodsAllocationId"))) {
                    depotItem.setAnotherGoodsAllocationId(rowObj.getLong("anotherGoodsAllocationId"));
                }
                //出库时判断库存是否充足
    ...
    public BigDecimal getFinishNumber(Long meId, Long id, Long headerId, Unit unitInfo, String materialUnit, String linkType) {
        ...
        if ("purchase".equals(linkType)) {
            ...
            //自营订单转自营入库
            if (BusinessConstants.SUB_TYPE_SELF_ORDER.equals(depotHead.getSubType())) {
                goToType = BusinessConstants.SUB_TYPE_SELF_IN_STORAGE;
            }
            //寄存订单转寄存入库
            if (BusinessConstants.SUB_TYPE_CONSIGNMENT_ORDER.equals(depotHead.getSubType())) {
                goToType = BusinessConstants.SUB_TYPE_CONSIGNMENT_IN_STORAGE;
            }
            //销售自营订单转销售自营出库
            if (BusinessConstants.SUB_TYPE_SALE_SELF_ORDER.equals(depotHead.getSubType())) {
                goToType = BusinessConstants.SUB_TYPE_SELF_OUTBOUND;
            }
            //销售寄存订单转销售寄存出库
            if (BusinessConstants.SUB_TYPE_SALE_CONSIGNMENT_ORDER.equals(depotHead.getSubType())) {
                goToType = BusinessConstants.SUB_TYPE_SALE_CONSIGNMENT_OUTBOUND;
            }
        }
