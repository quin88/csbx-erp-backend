...
public class DepotComponent implements ICommonQuery {
    ...
    private List<?> getDepotList(Map<String, String> map) throws Exception {
        ...
        String enabledFlag = StringUtil.getInfo(search, "enabledFlag");//启用查询标记，1:查询启用
        String order = QueryUtils.order(map);
        return depotService.select(name, type, remark, enabledFlag, QueryUtils.offset(map), QueryUtils.rows(map));
    }
    ...
    public Long counts(Map<String, String> map) throws Exception {
        ...
        String remark = StringUtil.getInfo(search, "remark");
        String enabledFlag = StringUtil.getInfo(search, "enabledFlag");//启用查询标记，1:查询启用
        return depotService.countDepot(name, type, remark, enabledFlag);
    }
